...
export class TodoService {
  ...
  getAll(options: {
    status?: string;
    project_id?: number;
    parent_id?: number | null;    // null이면 최상위 태스크만
    level?: string;
  } = {}): TodoListResult {
    ...
    // 프로젝트 필터
    if (options.project_id !== undefined) { query += ' AND project_id = ?'; params.push(options.project_id); }
    // 부모 필터 (null = 최상위 태스크, 숫자 = 특정 부모의 서브태스크)
    if (options.parent_id === null) { query += ' AND parent_id IS NULL'; }
    else if (options.parent_id !== undefined) { query += ' AND parent_id = ?'; params.push(options.parent_id); }
    // 레벨 필터
    if (options.level) { query += ' AND level = ?'; params.push(options.level); }

    query += ' ORDER BY priority ASC, due_date ASC';
    const todos = this.db.prepare(query).all(...params) as unknown as Todo[];
    return { todos, totalCount: todos.length };
  }

  /** 단독 투두 목록 (프로젝트 미소속) */
  getStandalone(options: { status?: string } = {}): TodoListResult {
  ...
  getProjectTree(projectId: number): TodoWithChildren[] {
    ...
    const tasks = this.db.prepare(
      'SELECT * FROM todos WHERE project_id = ? AND parent_id IS NULL ORDER BY priority ASC, due_date ASC'
    ).all(projectId) as unknown as Todo[];

    // 2. 각 태스크의 서브태스크 부착
    return tasks.map((task) => {
      const subtasks = this.db.prepare(
        'SELECT * FROM todos WHERE parent_id = ? ORDER BY priority ASC, due_date ASC'
      ).all(task.id) as unknown as Todo[];
  ...
  getById(id: number): Todo {
    const todo = this.db.prepare('SELECT * FROM todos WHERE id = ?').get(id) as unknown as Todo | undefined;
    if (!todo) throw new ServiceError(`투두를 찾을 수 없습니다 (ID: ${id})`, 404, 'TODO_NOT_FOUND');
    return todo;
  }

  /** 투두 생성 — 계층 자동 결정 */
  create(data: TodoCreateInput): Todo {
    const { title, description = null, priority = 3, due_date = null,
            project_id = null, parent_id = null, milestone_id = null } = data;
    ...
    const result = this.db.prepare(
      'INSERT INTO todos (title, description, priority, due_date, project_id, parent_id, milestone_id, level) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(title, description, priority, due_date, resolvedProjectId, parent_id, milestone_id, level);
  ...
  logAction(action: ToolAction): void {
    this.db.prepare(
      'INSERT INTO tool_actions (action_type, todo_id, payload, success, error_message) VALUES (?, ?, ?, ?, ?)'
    ).run(action.actionType, action.todoId, JSON.stringify(action.payload), action.success ? 1 : 0, action.errorMessage || null);
  }
}
