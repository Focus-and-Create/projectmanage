...
export function registerTodoTools(server: McpServer, todoService: TodoService): void {
    ...
    async ({ status, project_id, standalone }) => {
      ...
      if (standalone) {
        ...
        if (result.totalCount === 0) {
          return { content: [{ type: 'text' as const, text: '단독 투두가 없습니다.' }] };
        }
        const lines = result.todos.map((t) => formatTodo(t));
        return { content: [{ type: 'text' as const, text: `## 단독 투두 (${result.totalCount}개)\n\n${lines.join('\n')}` }] };
      }
      ...
      if (project_id) {
        ...
        if (tree.length === 0) {
          return { content: [{ type: 'text' as const, text: '이 프로젝트에 태스크가 없습니다.' }] };
        }
        ...
        return { content: [{ type: 'text' as const, text: `## 프로젝트 태스크\n\n${lines.join('\n')}` }] };
      }

      // 전체 투두
      const result = todoService.getAll({ status: statusFilter });
      if (result.totalCount === 0) {
        return { content: [{ type: 'text' as const, text: '투두가 없습니다.' }] };
      }
      const lines = result.todos.map((t) => formatTodo(t));
      return { content: [{ type: 'text' as const, text: `## 전체 투두 (${result.totalCount}개)\n\n${lines.join('\n')}` }] };
    }
  );
  ...
  - milestone_id (number): 마일스톤 ID (선택)`,
    ...
    async (params) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `투두 생성 실패: ${msg}` }], isError: true };
      }
    }
  );
  ...
  server.registerTool(
    ...
    async ({ id, ...data }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `투두 [${id}] "${todo.title}" 수정 완료.` }] };
      } catch (error) {
        const msg = error instanceof Error ? error.message : '오류';
        todoService.logAction({ actionType: 'modify', todoId: id, payload: { id, ...data }, success: false, errorMessage: msg });
        return { content: [{ type: 'text' as const, text: `수정 실패: ${msg}` }], isError: true };
      }
    }
  );
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `투두 [${id}] "${todo.title}" 삭제 완료.` }] };
      } catch (error) {
        const msg = error instanceof Error ? error.message : '오류';
        todoService.logAction({ actionType: 'delete', todoId: id, payload: { id }, success: false, errorMessage: msg });
        return { content: [{ type: 'text' as const, text: `삭제 실패: ${msg}` }], isError: true };
      }
    }
  );
  ...
  server.registerTool(
    ...
    async ({ id, undo }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `투두 [${id}] "${todo.title}" ${action}.` }] };
      } catch (error) {
        const msg = error instanceof Error ? error.message : '오류';
        todoService.logAction({ actionType: 'complete', todoId: id, payload: { id, undo }, success: false, errorMessage: msg });
        return { content: [{ type: 'text' as const, text: `상태 변경 실패: ${msg}` }], isError: true };
      }
    }
  );
    ...
    async ({ parent_id, subtasks }) => {
      try {
        ...
        if (parent.level === 'subtask') {
          return { content: [{ type: 'text' as const, text: '서브태스크 아래에 추가 하위를 만들 수 없습니다 (최대 3단계).' }], isError: true };
        }

        // 서브태스크 일괄 생성
        const created = todoService.createMany(
          subtasks.map((st) => ({
            title: st.title,
        ...
        return { content: [{ type: 'text' as const, text: `태스크 쪼개기 실패: ${msg}` }], isError: true };
      }
    }
  );
}
...
/** 투두 한 줄 포맷 */
function formatTodo(t: { id: number; title: string; status: string; priority: number; due_date: string | null; project_id: number | null }): string {
