...
// 프로젝트 + 마일스톤 관련 MCP 도구
import { z } from 'zod';
export function registerProjectTools(server, projectService, todoService) {
    // ─── project_list ────────────────────────────────────
...
Returns:
  프로젝트 목록 (이름, 상태, 진행률 포함)`,
        ...
        return { content: [{ type: 'text', text: `## 프로젝트 (${projects.length}개)\n\n${lines.join('\n\n')}` }] };
    });
    // ─── project_create ──────────────────────────────────
...
Returns:
  프로젝트 진행률, 마일스톤 현황, 태스크 트리`,
        ...
        try {
            ...
            return { content: [{ type: 'text', text: `프로젝트 조회 실패: ${msg}` }], isError: true };
        }
    });
    // ─── project_update ──────────────────────────────────
...
Returns:
  수정된 프로젝트 정보`,
        ...
        try {
            ...
            return { content: [{ type: 'text', text: `프로젝트 [${id}] "${project.name}" 수정 완료. 상태: ${project.status}` }] };
        }
        catch (error) {
            const msg = error instanceof Error ? error.message : '오류';
            return { content: [{ type: 'text', text: `프로젝트 수정 실패: ${msg}` }], isError: true };
        }
    });
    // ─── milestone_create ────────────────────────────────
...
Returns:
  생성된 마일스톤 정보`,
        ...
        try {
            ...
            return { content: [{ type: 'text', text: `마일스톤 생성: [${m.id}] "${m.title}" (목표: ${m.target_date})` }] };
        }
        catch (error) {
            const msg = error instanceof Error ? error.message : '오류';
            return { content: [{ type: 'text', text: `마일스톤 생성 실패: ${msg}` }], isError: true };
        }
    });
    // ─── milestone_complete ──────────────────────────────
...
Args:
  ...
  - undo (boolean): true이면 미완료로 되돌림 (기본: false)`,
        ...
        try {
            ...
            return { content: [{ type: 'text', text: `마일스톤 [${id}] "${m.title}" ${action}.` }] };
        }
        catch (error) {
            const msg = error instanceof Error ? error.message : '오류';
            return { content: [{ type: 'text', text: `마일스톤 처리 실패: ${msg}` }], isError: true };
        }
    });
}
// ─── 유틸리티 ──────────────────────────────────────────
